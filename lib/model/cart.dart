...
class CartSnapShot {
  static Map<String, dynamic> addOrder(
      double sum, String s, int code, int user) {
    return {
      'id': code,
      'user_id': user,
  ...
  static Future<void> insert(
      List<CartItem> list, int userId, List<String> address) async {
    ...
    await supabase
        .from("order")
        .insert(CartSnapShot.addOrder(sum, addr, code, userId))
        .then(
          (value) => print("đã thêm order"),
        )
        .catchError((error) {});
    for (CartItem i in list) {
      await supabase
          ...
          .insert(i.toMap(code))
          .then(
            (value) => print("đã thêm order_item"),
          )
          .catchError((error) {});
    }
  }
